...
def addref(caption, outstream):
    ...
    print(r"%s" % caption, file=outstream)
...
def processwithcomments(caption, instream, outstream, listingslang):
    ...
    out = []
    if warning:
        print(f"WARNING: ({caption}, {warning})")
    if error:
        print(f"ERROR: ({caption}, {error})")
    else:
        # addref(caption, outstream)
        if commands.get("Description"):
            out.append(r"# Description: %s" % snippet_escape(commands["Description"]))
        if commands.get("Usage"):
            out.append(r"# Usage: %s" % snippet_escape(commands["Usage"]))
        if commands.get("Time"):
            out.append(r"# Time: %s" % snippet_escape(commands["Time"]))
        if commands.get("Memory"):
            out.append(r"# Memory: %s" % snippet_escape(commands["Memory"]))
        if includelist:
            out.append(r"# include: %s" % snippet_escape(", ".join(includelist)))
        # if nsource:
        ...
        escaped_caption = pathescape(caption).strip()
        out.append(f"snippet {escaped_caption} {escaped_caption}")
        # out.append(nsource)
    ...
    for line in nsource.split("\n"):
        print(f"    {line}", file=outstream)
